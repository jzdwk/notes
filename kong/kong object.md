...
一个service代表一个微服务实体，可以简单理解为一个网站的根地址（www.test.com）,对于k8s场景来说，就是应用的svc，或者说是ingress中描述的host，service中最重要的属性是url(protocol,hsot,port,path)。service定义后，通过不同的route进行访问。
service创建例子如下：
...
1. curl -i -X GET http://www.test.com/req1 这个请求，我们定义了一个url是http://http://www.test.com/req1的service，当然，我们还需要定义一个path是req1的route。
